...
from typing import Union
def synthesize_phrase_audio(text: str, language: str) -> Union[BytesIO, None]:
...
class LocalPhraseRecognizer:
    def __init__(self):
        ...
        # Prefer new TCN both-hands model, fallback to legacy LSTM
        root = Path(__file__).resolve().parent.parent.parent
        candidates = [root / "isl_model_tcn.keras", root / "isl_model.keras", root / "isl_model.h5"]
        model_path = None
        for c in candidates:
            if c.exists():
                model_path = c
        ...
        if model_path is None:
            raise FileNotFoundError(f"Could not find any phrase model at {candidates}")
        ...
        # Infer expected feature dimension (63=single hand, 126=both hands)
        try:
            ishape = getattr(self.model, 'input_shape', None)
            while isinstance(ishape, (list, tuple)) and len(ishape) == 1:
                ishape = ishape[0]
            self._feature_dim = int(ishape[-1]) if ishape else 63
        except Exception:
            self._feature_dim = 63
        self._use_both_hands = self._feature_dim >= 100
    ...
    def _safe_load_model(self, path: Path):
        """Robust loader that tries multiple backends and disables compile to avoid deserialization issues."""
        last_err = None
        # Try tf.keras with compile=False first
        try:
            return tf_load_model(str(path), compile=False)
        except Exception as e:
            last_err = e
        # Try tf.keras default
        try:
            return tf_load_model(str(path))
        except Exception as e:
            last_err = e
        # Try keras (standalone, if available)
        try:
            import keras
            return keras.models.load_model(str(path))
        except Exception as e:
            last_err = e
        # Fallback: if .h5 sibling exists, try it
        try:
            alt = path.with_suffix('.h5')
            if alt.exists():
                return tf_load_model(str(alt), compile=False)
        except Exception as e:
            last_err = e
        raise RuntimeError(f"Unable to load phrase model at {path}: {last_err}")
    ...
    def extract_keypoints(self, results):
        """
        Return features to match model expectation:
        - 63 dims: single hand (prefer right, else left, else zeros)
        - 126 dims: concatenate right(63) then left(63), zeros if missing
        """
        def to63(hand):
            if not hand:
                return np.zeros(21 * 3, dtype=np.float32)
            return np.array([[res.x, res.y, res.z] for res in hand.landmark], dtype=np.float32).reshape(-1)
        if self._use_both_hands:
            right = to63(results.right_hand_landmarks)
            left = to63(results.left_hand_landmarks)
            return np.concatenate([right, left]).astype(np.float32)
        # single-hand fallback
        if results.right_hand_landmarks:
            return to63(results.right_hand_landmarks)
        if results.left_hand_landmarks:
            return to63(results.left_hand_landmarks)
        return np.zeros(21 * 3, dtype=np.float32)
    ...
    def process_frame(self, frame):
        ...
        alternatives = []
        # Initialize EMA buffer for smoothed probabilities on first call
        if not hasattr(self, "_prob_ema"):
            self._prob_ema = None
            self._ema_alpha = 0.3
        if results.left_hand_landmarks or results.right_hand_landmarks:
            ...
            if len(self.sequence) == 30:
                ...
                prob = self.model.predict(input_seq, verbose=0)[0]
                # Exponential moving average to reduce jitter
                if self._prob_ema is None:
                    self._prob_ema = prob.copy()
                else:
                    self._prob_ema = self._ema_alpha * prob + (1 - self._ema_alpha) * self._prob_ema
                res = self._prob_ema
...
def main():
    ...
    if 'words_camera_permission_granted' not in st.session_state:
        st.session_state.words_camera_permission_granted = False
    if 'words_camera' not in st.session_state:
        st.session_state.words_camera = None
    prev_perm = bool(st.session_state.words_camera_permission_granted)
    perm_c1, perm_c2 = st.columns([2, 1])
    with perm_c1:
        st.caption("Use the toggle to grant or revoke camera access.")
    with perm_c2:
        st.toggle("Camera access", key="words_camera_permission_granted")
    curr_perm = bool(st.session_state.words_camera_permission_granted)
    if curr_perm and not prev_perm:
        ...
        if st.session_state.get("words_camera_checkbox"):
            st.session_state.words_camera_checkbox = False
        cam = st.session_state.get('words_camera')
        if cam is not None:
            try:
                cam.release()
            except Exception:
    ...
    run = st.checkbox("Start camera", key="words_camera_checkbox", disabled=not st.session_state.words_camera_permission_granted)

    if run:
        recognizer.reset()
        recognizer.ensure_active()
        st.session_state.words_camera = cv2.VideoCapture(0)
        camera = st.session_state.words_camera
        ...
        try:
            ...
            cam = st.session_state.get('words_camera') or camera
            try:
                if cam is not None:
                    cam.release()
            except Exception:
